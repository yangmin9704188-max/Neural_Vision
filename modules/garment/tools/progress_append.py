...
def main() -> int:
    ...
    parser.add_argument("--step", required=True, dest="step_id", help="e.g. G01, G02 (UNSPECIFIED rejected)")
    ...
    parser.add_argument("--module", default="garment")
    parser.add_argument("--warnings", type=int, default=None, help="warnings count (optional)")
    parser.add_argument("--phase", default=None, help="phase (optional)")
    parser.add_argument("--run-id", default=None, dest="run_id", help="run_id (optional)")
    args = parser.parse_args()

    if not _is_valid_step_id(args.step_id):
        print("progress_append: step_id required and must be Gxx/Fxx (UNSPECIFIED rejected). Exit 1.", file=sys.stderr)
        return 1
    ...
    ev = {
        ...
        "event": args.event,
        "run_id": args.run_id if args.run_id else "N/A",
        "status": args.status,
        "dod_done_delta": args.dod_done_delta,
        "note": args.note,
        "evidence": [],
        "warnings": [f"count={args.warnings}"] if args.warnings is not None and args.warnings > 0 else [],
    }
